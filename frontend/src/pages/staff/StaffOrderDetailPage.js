// File: frontend/src/pages/staff/StaffOrderDetailPage.js
import React, { useState, useEffect, useCallback } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import toast from 'react-hot-toast';
import apiClient from '../../services/apiClient';
import { formatNumber } from '../../utils/formatNumber';
import { FaCoins, FaDollarSign, FaCheck, FaTimes, FaClock, FaCreditCard, FaArrowLeft } from 'react-icons/fa';
import OrderStatusStepper from '../../components/order/OrderStatusStepper';

// Helper function - same as admin
...
const getNextStatus = (current) => {
    const flow = ['PENDING', 'PREPARING', 'READY_FOR_DELIVERY', 'COMPLETED'];
...
export default function StaffOrderDetailPage() {
    ...
    const handleAdvanceStatus = async () => {
        setIsUpdating(true);
        try {
            await apiClient.post(`/orders/admin/${orderId}/advance`);
    ...
    // Xác nhận thanh toán và hoàn thành
    const handleConfirmComplete = async () => {
        setIsUpdating(true);
        try {
            await apiClient.post(`/orders/admin/${orderId}/confirm-complete`);
    ...
    const handleCancelOrder = async () => {
        ...
        setIsUpdating(true);
        try {
            await apiClient.patch(`/orders/admin/${orderId}`, { status: 'CANCELLED', cancelReason: reason });
    ...
    const handleConfirmPayment = async () => {
        setIsUpdating(true);
        try {
            await apiClient.patch(`/orders/admin/${orderId}`, { paymentStatus: 'PAID' });
    ...
    if (error) {
        return (
            <div className="text-center">
                <p className="text-lg text-red-400 mb-4">Lỗi: {error}</p>
                <button onClick={() => navigate('/staff/orders')} className="text-blue-400 hover:text-blue-300">
                    ← Quay lại danh sách
                </button>
            </div>
        );
    ...
    return (
        <div>
            <button onClick={() => navigate('/staff/orders')} className="mb-4 text-blue-400 hover:text-blue-300 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-gray-900 shadow-xl rounded-lg p-6">
                    ...
                    <p className="font-mono text-xl font-bold text-blue-400 mb-2">
                    ...
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold text-blue-400 mb-3">Khách hàng</h2>
                    ...
                    {/* Chi tiết vật phẩm */}
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold text-blue-400 mb-3">Vật phẩm đã đặt</h2>
